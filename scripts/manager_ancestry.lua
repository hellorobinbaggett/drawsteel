...
DEFAULT_BOOK_INDEX = "reference.ancestryindex";
DEFAULT_BOOK_CONTENT = "reference.ancestrydata";
...
function addStoryIndexRecord(node, bInit)
	...
	local tRecord = {};
	tRecord.vNode = node;
	tRecord.sCategory = UtilityManager.getNodeCategory(node);
...
function handlePagePrev(w, sRecord)
	...
	local sModule = DB.getModule(sRecord);
	local sPrevPath = AncestryManager.getStoryPrevRecord(sModule, sRecord, true) or "";
	if sPrevPath ~= "" then
		AncestryManager.activateLink(w, nil, sPrevPath);
	end
end
function handlePageNext(w, sRecord)
	...
	local sModule = DB.getModule(sRecord);
	local sNextPath = AncestryManager.getStoryNextRecord(sModule, sRecord, true) or "";
	if sNextPath ~= "" then
		AncestryManager.activateLink(w, nil, sNextPath);
	end
end
...
function onBookIndexMoveUp(w)
	...
	if sClass == "story_book_index_chapter" then
		...
		if nOrder > 1 then
			AncestryManager.setWindowOrderValue(tOrderedChildren[nOrder - 1], nOrder);
			AncestryManager.setWindowOrderValue(tOrderedChildren[nOrder], nOrder - 1);
...
function updateBlockImageControls(wBlock, bReadOnly)
	if wBlock.image then
		local bHasImageLink = AncestryManager.getBlockImageLinkBool(wBlock);
...
function addBlockImage(wBlock, sAlign)
	local node = wBlock.getDatabaseNode();
	local sAsset, wImage, hImage = AncestryManager.getBlockImageData(wBlock, sAlign);
...
function addBlockIcon(wBlock, sAlign)
	local node = wBlock.getDatabaseNode();
	local sIcon, wImage, hImage = AncestryManager.getBlockIconData(wBlock, sAlign);
