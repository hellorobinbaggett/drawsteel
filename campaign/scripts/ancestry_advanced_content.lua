...
function onInit()
	AncestryManager.initRecordLegacyText(self);
	AncestryManager.onRecordRebuild(self);
	self.update();
end
...
function update()
	...
	if not bReadOnly then
		AncestryManager.migrateRecordLegacyTextToBlock(self);
	end
	
	for _,wBlock in pairs(blocks.getWindows()) do
		AncestryManager.onBlockUpdate(wBlock, bReadOnly);
	end
